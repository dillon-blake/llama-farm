...
at the project's GitHub fork branch `learning-llamas-base`, with a `patches/` queue mechanism and
...
**private `src/` internals** (S0-03), so the exact llama.cpp commit is part of learning-llamas's ABI,
...
this ticket implements: a GitHub fork with a `learning-llamas-base` branch pinned at `4f37f51`,
...
checkout at `/home/dillon/Desktop/learning-llamas/llama.cpp` is **clean** (`git status` empty) at
...
should be empty: real changes live as commits on `learning-llamas-base`.

## What to do

1. Create the GitHub fork of `ggml-org/llama.cpp` under the project owner's account. Create
   branch `learning-llamas-base` pointing exactly at `4f37f519722aa3242eecb7649466b4a4a2d6d6da`;
...
2. Verify lineage and record evidence: from the local checkout, `git fetch` upstream and confirm
   ...
   for the ADR.
3. In learning-llamas, `git submodule add <fork-url> vendor/llama.cpp`, checked out at `4f37f51` and
   tracking `learning-llamas-base`. Commit `.gitmodules` + the gitlink.
4. `patches/` mechanism: `patches/README.md` (format: numbered `git format-patch` files, applied
   ...
   `learning-llamas-base`.
5. Write `docs/adr/ADR-0001-vendor-lineage.md`: the lineage finding from step 2 (with evidence);
   the decision to pin the fork's `learning-llamas-base` at `4f37f51`; the ROADMAP §11 triage classes
...
6. Document the two-repo PR flow for kernel tickets in ADR-0001 (matching `tickets/README.md`):
   implementation PR against the fork's `learning-llamas-base` with the ticket ID in the title, then
   a trivial learning-llamas PR bumping the `vendor/llama.cpp` gitlink and referencing the same
   ticket ID.
...
- [ ] `.gitmodules` URL points at the project fork (not `ggml-org/llama.cpp`), and branch
      `learning-llamas-base` exists on the fork at that commit (URL recorded in ADR-0001).
...
- One PR in learning-llamas; the fork/branch creation itself happens on GitHub and is referenced, not
  diffed.
