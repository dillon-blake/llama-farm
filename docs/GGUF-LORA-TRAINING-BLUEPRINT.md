...
- Training forward: `logp_new` via `ce_sparse`; importance ratio `exp(logp_new − logp_old_const)` (EXP VJP exists); **PPO clip composed from RELU identities** — `clip(r,lo,hi) = lo + relu(r−lo) − relu(r−hi)`, `min(a,b) = a − relu(a−b)` — because `CLAMP` has no backward (a ~10-line upstream VJP would simplify; do both); per-token weighting = `mul` by `advantage·mask` input; optional KL penalty to the reference policy — the low-variance "k3" estimator `exp(Δ) − Δ − 1`, built as `expm1(Δ) − Δ` — from precomputed ref logprobs via EXP/SUB.
  - ⚠️ **The `min` must be `a − relu(a−b)`, not `b − relu(b−a)`.** The two are equal as *numbers* and differ as *gradients*, because `ggml_step(0) == 0`. At a tie (`a == b`, i.e. exactly at the clip bound) the `relu` term contributes zero derivative, so whichever argument sits *outside* the `relu` is the one the gradient flows through. Writing it `b − relu(b−a)` routes the tie into the **clipped** branch — whose derivative in `r` is zero — and the gradient silently vanishes. Measured on the real graph: 3 ulps below the bound the adapter moves `5.0e-01`; exactly *at* the bound it moves `0.0e+00`; 3 ulps above, `5.0e-01` again — **with an identical loss in all three cases**. An earlier revision of this document prescribed the broken form; S1-16 had to reject it. See `docs/dev/grpo.md`.
  - The `min` is taken **after** multiplying by the advantage, and it has to be: a negative advantage swaps the two arguments, so the clip binds from the other side, and PPO's entire asymmetry lives in that swap.
